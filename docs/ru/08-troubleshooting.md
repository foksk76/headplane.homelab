Язык: [English](../08-troubleshooting.md) | [Русский](08-troubleshooting.md)
...
## Не совпадает адрес возврата при входе через поставщика удостоверений

Причина:

- адрес возврата у поставщика удостоверений не совпадает в точности с
  публичным адресом возврата Headplane
...
## OIDC все еще настроен неверно после копирования примера из более новых документов

Причина:

- в установку `v0.6.2` был перенесен вариант с `headscale.api_key` из более
  новых формулировок документации
- сама версия `v0.6.2` по-прежнему ожидает ключ API для OIDC внутри секции
  `oidc`

Исправление:

- для `v0.6.2` используйте `oidc.headscale_api_key` или
  `oidc.headscale_api_key_path`
- при включении OIDC явно оставляйте `oidc.enabled: true`
...
Назад: [Обновление Headplane](07-upgrade-headplane.md) | Вперед: [Быстрый старт](../../README.ru.md#быстрый-старт)
