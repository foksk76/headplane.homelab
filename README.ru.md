...
> **Статус проекта:** рабочие заметки по установке Headplane `v0.6.2`,
> проверенные на промежуточном хосте сборки и Debian 13 VPS. По состоянию на
> 13 мая 2026 года `v0.6.2` остается последним стабильным релизом Headplane,
> на который и опирается этот репозиторий. В текущую проверку уже входит живой
> откат из резервной копии, сделанной до включения OIDC, включая ожидаемую
> инвалидизацию более новых ключей API Headscale после восстановления
> `db.sqlite`.
...
- [вход через OIDC в Headplane](https://headplane.net/features/sso)
...
7. Проверить локальные и внешние HTTP-пути и войти либо через ключ API
   Headscale, либо через встроенный OIDC-механизм Headplane.
8. Перед правкой входа или прокси держать под рукой свежий архив для отката и
   помнить, что ключи API, созданные после этой точки, после восстановления
   базы погаснут. Не баг, а механика.
...
- нужен простой запасной вход по ключу API и отдельный штатный путь входа через
...
- `docs/ru/06-backup-and-restore.md` — резервное копирование и восстановление,
  включая очистку более новых OIDC-артефактов перед откатом
- `docs/ru/07-upgrade-headplane.md` — проверка последнего стабильного релиза и
  обновление более старой установки до `v0.6.2`
- `docs/ru/08-troubleshooting.md` — диагностика
...
7. [Обновление Headplane](docs/ru/07-upgrade-headplane.md)
8. [Диагностика](docs/ru/08-troubleshooting.md)
...
- [Обновление Headplane](docs/ru/07-upgrade-headplane.md)

Если позже понадобится откатиться на более ранний архив, сначала откройте
инструкцию по восстановлению. Восстановление базы Headscale откатывает и ключи
API, поэтому все ключи, созданные уже после резервной копии, должны перестать
работать.
...
- `ключ API Headscale` — самый быстрый путь для первичного входа и аварийного
  административного доступа
- `OIDC в Headplane` — штатный путь единого входа через внешнего поставщика
  удостоверений

Если OIDC включен, Headplane использует свой встроенный сценарий входа и при
этом сохраняет вход по ключу API как запасной административный вариант.
